		...
		<xsl:for-each select="module">		
			...
			<P/><H3 class="dtH3"><A>
				...
				<div class="tablediv"><table cellspacing="0" class="dtTABLE">								
					...
					<xsl:sort order="ascending" select="name"/>
					<tr>
					<td align="left"><A><xsl:attribute name="href">#OT<xsl:number from="/"/><xsl:value-of select="../name"/></xsl:attribute><xsl:value-of select="name"/></A></td>																				
						<td align="left"><xsl:value-of select="newname"/></td>
					</tr>					
					</xsl:for-each>								
				</table></div>															
	...
	<xsl:template match="mapping" mode="typedetails">									
		<xsl:for-each select="module">					
         ...
         <P/>								
			...
			<P/><A><xsl:attribute name="href">#OM<xsl:number from="/"/><xsl:value-of select="./../name"/></xsl:attribute>View Members by Name</A>
